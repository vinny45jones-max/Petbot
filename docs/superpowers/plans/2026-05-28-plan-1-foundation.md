...
(Подобную структуру для `app/(payload)/api/[...slug]/route.ts` — взять из официального Payload Next.js template; команда `npx create-payload-app@latest --next` может скопировать референс).
...
export async function POST(req: NextRequest) {
  ...
  const login = await payload.login({ collection: 'users', data: { email: user.email, password: '' }, req: req as any }).catch(() => null);
  const token = login?.token;
  const res = NextResponse.json({ ok: true, user: { id: user.id, firstName: user.firstName, role: user.role } });
  if (token) {
...
(Логин через `payload.login` без пароля для TG-пользователей будет дополнен в Task 12 через `account-linking`.)
...
`web/app/(public)/forgot-password/page.tsx` и `reset-password/page.tsx` — аналогичные формы с использованием Payload built-in `/api/users/forgot-password` и `/api/users/reset-password`.
...
export async function GET(req: NextRequest) {
  ...
  // Создаём сессионный cookie через Payload
  const userId = typeof entry.user === 'string' ? entry.user : entry.user.id;
  const cookie = await payload.encryptCookie?.({ userId }) || null;
  const res = NextResponse.redirect(new URL('/me', req.url));
  if (cookie) res.cookies.set('payload-token', cookie, { httpOnly: true, sameSite: 'lax', secure: process.env.NODE_ENV === 'production', maxAge: 60 * 60 * 24 * 30 });
  return res;
}
```

(Точный API создания cookie может потребовать использования Payload `payload.forgotPassword`/`resetPassword`-стиля токена или `jwt.sign` с PAYLOAD_SECRET — детали зависят от версии Payload; адаптировать при имплементации.)
