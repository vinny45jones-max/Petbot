...
**Фаза 1:** User, Organization, Animal, City, Media, BlogPost, LegalArticle, Donation, AdoptionInquiry, Comment, ReportFlag, CrueltyReport, NotificationPreference, HelpArticle, Glossary, AuditLog, **IntakeFacility**.
...
## 17. Critical-vertical: муниципальные службы отлова и обратный отсчёт до эвтаназии

### 17.1 Проблема

Муниципальные службы отлова безнадзорных животных (например, ГУ «Фауна города» на Гурского 47 в Минске, аналогичные службы в областных центрах) содержат отловленных животных **ограниченный срок**. После истечения срока животное по закону может быть подвергнуто эвтаназии, если хозяин или новый владелец не нашлись.

Эти животные — самая срочная группа на платформе. Их нужно выделять, показывать обратный отсчёт, поднимать в выдаче и активно рассылать алерты.

### 17.2 Нормативная база (фиксируется в фазе 0 юристом-партнёром)

Срок содержания регулируется:

- **Закон РБ № 361-З «Об ответственном обращении с животными»** (в силе с 01.01.2025)
- **Правила содержания и регулирования численности собак, кошек** (постановление Совмина / Минжкх / Минсельхоза)
- Местные нормативные акты Мингорисполкома и областных исполкомов

В spec'е НЕ фиксируем конкретное число дней — оно может отличаться по регионам и регулярно меняется. **Хранится как поле `IntakeFacility.legal_hold_days` + override в `Animal.legal_deadline_date`** на случай индивидуальных решений.

### 17.3 Новая сущность IntakeFacility (фаза 1)

```
IntakeFacility
- id
- name (например, "ГУ Фауна города (Минск, Гурского 47)")
- slug
- city_id → City
- address, phone, email
- legal_hold_days  — сколько дней по закону держат до возможной эвтаназии
- description (rich text)
- contact_tg_url, viber_url
- is_municipal (bool) — отличать муниципальные службы от частных приютов
- is_published
```

Seed-данные: ~6-10 крупнейших служб РБ (Минск, Брест, Витебск, Гомель, Гродно, Могилёв) с подтверждёнными контактами.

### 17.4 Расширение Animal под эту вертикаль

Добавить в `Animal`:

- `intake_facility_id` → IntakeFacility (nullable) — для животных из служб отлова
- `intake_date` (уже есть — дата попадания)
- `legal_deadline_date` (nullable) — рассчитывается автоматически: `intake_date + IntakeFacility.legal_hold_days`. Можно override вручную модератором.
- `urgency_level` enum: `normal | high | critical` — пересчитывается ежедневно cron'ом по `days_until_deadline`:
  - `critical` если ≤3 дня
  - `high` если ≤7 дней
  - `normal` иначе
- `is_at_risk` (computed) — true если `legal_deadline_date` существует и в будущем

### 17.5 UX / UI для этой вертикали

**На карточке животного:**

- Большой красный бейдж `СРОЧНО · Осталось 3 дня` (countdown) — если `urgency = critical`
- Оранжевый бейдж `Срочно · Осталось 5 дней` — если `urgency = high`
- Блок «В службе отлова» с названием facility, адресом, телефоном, кнопкой «Позвонить»
- CTA: **«Забрать сейчас»** вместо стандартного «Хочу взять домой» (с инструкцией звонка в facility)

**На каталоге `/animals`:**

- Дефолтная сортировка: `critical` → `high` → новые
- Фильтр «Только срочные» (checkbox)
- Бейджи на карточках в каталоге

**Отдельная страница `/animals/urgent`:**

- Все животные с `urgency in (critical, high)`
- Группировка по городам и facility
- Большие фото, обратный отсчёт
- Shareable URL для распространения в соцсетях

**Главная страница:**

- Если есть `critical`-животные — отдельный hero-блок «Срочно нужна помощь — осталось менее 3 дней» с топ-3
- Не показывать если таких животных нет

### 17.6 Уведомления и автопостинг

**Cron `urgency_recalc_daily`:**

- Раз в сутки пересчитывает `urgency_level` у всех `Animal.intake_facility_id IS NOT NULL`
- Triggers алерты ниже

**Email-алерты подписчикам:**

- В `NotificationPreference` добавить флаг `notify_urgent_in_city` (выбор городов)
- Когда у животного `urgency` переходит в `critical` → email подписчикам
- Daily digest «Срочные сегодня» опционально

**Auto-post в Telegram-канал проекта:**

- При переходе в `critical` — пост в канал с фото и счётчиком
- При переходе в `high` — опционально, по настройке
- При `adoption_inquiry` или `adopted` — апдейт-пост «спасён»

**Pet BOT интеграция (фаза 2):**

- Бот шлёт срочные алерты в свой канал автоматически
- В фазе 1 — ручной репост модератором

### 17.7 Источник данных по службам отлова

Главный вопрос — **где брать данные о новых животных в отлове**:

**Модель А (фаза 1):** ручной ввод через partner-кабинет

- Договорённость со службой → им создаётся `Organization`-связь + `IntakeFacility` запись
- Их сотрудник в роли `org_admin` загружает фото и анкету как обычный приют, но с заполненным `intake_facility_id`
- `legal_deadline_date` рассчитывается автоматически из `IntakeFacility.legal_hold_days`

**Модель B (фаза 2):** партнёрский импорт

- Если у facility есть свой сайт / страница / Google Sheets — webhook или scheduled import
- Дубль-детекция

**Модель C (фаза 3):** видео-камеры в facility (как Teddy Food)

- Real-time трансляция из вольеров (по согласию facility)
- Donations на содержание конкретного животного (продление срока за счёт спонсорства если разрешено)

### 17.8 Юр.раздел: добавить статью

Новая `LegalArticle`:

- **slug:** `municipal-intake-rights`
- **title:** «Что делать если ваше животное попало в службу отлова»
- **body:** инструкция шаг-за-шагом: куда звонить, какие документы взять, сроки востребования, как доказать собственность (чип, фото, ветпаспорт)
- **npa_links:** ссылки на 361-З, постановления о содержании безнадзорных

### 17.9 Митигация рисков

| Риск | Митигация |
|---|---|
| Служба отлова не соглашается публиковаться у нас | Личные переговоры с руководством. Аргумент: больше adoption = меньше эвтаназий = снижение нагрузки на службу. |
| Сроки в законе меняются | `legal_hold_days` хранится в БД (а не в коде), модератор обновляет |
| Animal с `intake_facility` но без фото = не привлечёт adoption | Минимальный требование при создании: 1 фото + минимум описания. Модератор проверяет. |
| Spam-алерты «срочно» утомляют подписчиков | Throttle: максимум 1 email в день на пользователя с дайджестом всех `critical` |
| Юр.претензии «вы давите на службу отлова» | Нейтральная подача: «осталось N дней по законодательным правилам содержания», ссылка на 361-З, без обвинений |
| Animal не пришёл на сайт вовремя, и его уже усыпили | Ручной партнёрский канал + дублирование в чат-боте на Telegram для facility-сотрудников; SLA в договоре |

### 17.10 Изменения по сравнению с предыдущими секциями

- §6 (модель данных) → добавляется `IntakeFacility`, в `Animal` добавляются 3 поля
- §5 (карта страниц) → добавляется `/animals/urgent`, `/intake-facilities`, `/intake-facilities/[slug]`
- §10 (фазы 2-3) → расширения по импорту и видеотрансляциям
- §16.7 (финальный scope MVP) → в MVP теперь входит критическая вертикаль отлова
